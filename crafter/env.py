...
class Env(BaseClass):
    ...
    def __init__(
        ...
        self._step = None
        self._steps_in_day = 30
        self._steps_until_midnight = int(self._steps_in_day * (3 / 4))  # int для округления
        self._steps_until_midday = self._steps_in_day // 4
        self._player = None
    ...
    def _update_time(self):
        ...
        progress = (self._step / self._steps_in_day) % 1 + 0.3
        daylight = 1 - np.abs(np.cos(np.pi * progress)) ** 3
        self._world.daylight = daylight
        self._steps_until_midnight -= 1
        self._steps_until_midday -= 1

        if self._steps_until_midnight == 0:
            self._steps_until_midnight = self._steps_in_day
        if self._steps_until_midday == 0:
            self._steps_until_midday = self._steps_in_day

        print("step: ", self._step)
        print("progress: ", progress)
        print("daylight: ", daylight)
        print("steps_until_midday: ", self._steps_until_midday)
        print("steps_until_midnight: ", self._steps_until_midnight)
